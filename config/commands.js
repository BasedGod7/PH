...
var commands = exports.commands = {
	...
	liga: function (target, room, user) {
		...
		buffer += '(LoD)Marcos: Acero<br><br>';
		buffer += '<b><font color="red">LÍDERES DE GIMNASIOS</font></b><br>';
		buffer += '<a href=' + path + 502 + '><button>Saigonthrasher: Normal</button></a>   ';
		buffer += '<a href=' + path + 507 + '><button>2020victini2: Hielo</button></a>   ';
		buffer += '<a href=' + path + 512 + '><button>CH Kaast: Agua</button></a>   ';
		buffer += '<a href=' + path + 514 + '><button>Spirit Virgo: Lucha</button></a>   ';
		...
		buffer += '<a href=' + path + 000 + '><button>(A/M) Roy: Volador</button></a>   ';
		buffer += '<a href=' + path + 535 + '><button>CSS Dedenneitor: Fantasma</button></a>   ';
		buffer += '<a href=' + path + 539 + '><button>Sidaaaaa: Tierra</button></a>   ';
		buffer += '<a href=' + path + 508 + '><button>(CC)Mr.Poke: Siniestro</button></a>   ';
		buffer += '<a href=' + path + 532 + '><button>Smorales: Veneno</button></a>   ';
		...
		return this.sendReply('|raw|<div class="infobox league-info">' + buffer + '</div>');
	},
	...
	help: function (target, room, user) {
		...
		if (!target) {
			...
			this.sendReply("COMANDOS DE INFORMACIÓN: /data, /dexsearch, /groups, /opensource, /avatars, /faq, /rules, /intro, /tiers, /othermetas, /learn, /analysis, /calc (replace / with ! to broadcast. (Requires: + % @ & ~))");
